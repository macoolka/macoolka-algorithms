...
describe('Graph', () => {
  it('should add vertices to graph', () => {
    ...
    expect(graph.getVertexByKey(vertexA.getKey())).toEqual(O.some(vertexA));
    expect(graph.getVertexByKey(vertexB.getKey())).toEqual(O.some(vertexB));
  });
  ...
  it('should add edges to undirected graph', () => {
    ...
    expect(O.isNone(graph.findVertexByKey('not existing'))).toBeTruthy();

    expect(O.option.map(graphVertexA,a=>a.getNeighbors().length)).toEqual(O.some(1));
    expect(O.option.map(graphVertexA,a=>a.getNeighbors()[0])).toEqual(O.some(vertexB));
    expect(O.option.map(graphVertexA,a=>a.getNeighbors()[0])).toEqual(graphVertexB);


    expect(O.option.map(graphVertexB,a=>a.getNeighbors().length)).toEqual(O.some(1));
    expect(O.option.map(graphVertexB,a=>a.getNeighbors()[0])).toEqual(O.some(vertexA));
    expect(O.option.map(graphVertexB,a=>a.getNeighbors()[0])).toEqual(graphVertexA);
  });
  ...
  it('should add edges to directed graph', () => {
    ...
    expect(O.option.map(graphVertexB,a=>a.getNeighbors().length)).toEqual(O.some(0));
  ...
  it('should find edge by vertices in undirected graph', () => {
    ...
    expect(O.isNone(graphEdgeAC)).toBeTruthy();
    expect(O.isNone(graphEdgeCA)).toBeTruthy();
    expect(graphEdgeAB).toEqual(O.some(edgeAB));
    expect(graphEdgeBA).toEqual(O.some(edgeAB));
    expect(O.option.map(graphEdgeAB,a=>a.weight)).toEqual(O.some(10));
  });
  ...
  it('should find edge by vertices in directed graph', () => {
    ...
    expect(O.isNone(graphEdgeCA)).toBeTruthy();
    expect(graphEdgeAB).toEqual(O.some(edgeAB));
    expect(O.isNone(graphEdgeBA)).toBeTruthy();
    expect(O.option.map(graphEdgeAB,a=>a.weight)).toEqual(O.some(10));
